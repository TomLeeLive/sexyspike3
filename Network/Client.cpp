...
INT	CClient::ClientRoom(PACKETDATA* pPacket)
{
	...
	switch(pPacket->EXETYPE)		
	{
		...
		case CLIENT_ROOM_POSITION_RESET:
			break;
		case CLIENT_ROOM_GAME_START:
			GMAIN->m_MultiPlay->m_MyInfo.State = CLIENT_PLAY;
			break;
		case CLIENT_ROOM_PLAY:
			...
			case U_CHARACTER:
				{
					PlayerPacket	*p = (PlayerPacket*)pPacket->DATA;
					for( int i=0; i<NUMPLAYERS; ++i )
					{
						CPlayer	*player	= (CPlayer*)g_ppPlayers[i];
						if( player->operator ==(p->Name) )
						{
							player->UpdatePlayerStateFromNetwork(p);
							break;
						}
					}
				}
			...
			case U_BALL:
				{
					CBall *pBall	= GMAIN->m_pBall;
					BallPacket *p	= (BallPacket*)pPacket->DATA;
					pBall->UpdateBallStateFromNetwork(p);
					g_CntBallTouched	= p->CNTTOUCHED;
					g_bNowServe			= p->NOWSERVER;
					if( NULL != p->Name[0] )
					{
						for( int i=0; i<NUMPLAYERS; ++i )
						{
							...
							if( player->operator ==(p->Name) )
							{
								g_pPlayerLastTouch = g_ppPlayers[i];
								break;
							}
						}
					}
				 ...
				 ,BYTE  DATASIZE,   void*  pData )
{
	...
	int	RoomIndex = GMAIN->m_MultiPlay->m_MyInfo.RoomNumber;

	PacketAssemble(Pakcet,INDEX,STATE, EXETYPE, RoomIndex,NAME,ORDER, DEFINITION,DATASIZE,  pData );
