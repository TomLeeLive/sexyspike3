        ...
        for( DWORD i=0; i<m_dwNumMaterials; i++ )
        {
            ...
            if( d3dxMtrls[i].pTextureFilename )
            {
                ...
                if( FAILED( D3DXCreateTextureFromFile( pd3dDevice, strTexture, 
                                                       &m_pTextures[i] ) ) )
				{
					GetCurrentDirectory( MAX_PATH, strTexture );
					int length	= lstrlen(strFilename);
					for( int s=length; s>=0; --s )
					{
						if( strFilename[s] == TEXT('\\') )
						{
							memset(strTextureTemp, 0, sizeof(TCHAR)*MAX_PATH);
							memcpy(strTextureTemp, strFilename, sizeof(TCHAR)*(s+1));
							lstrcat(strTexture, strTextureTemp);
							lstrcat(strTexture, d3dxMtrls[i].pTextureFilename);
							if( FAILED( D3DXCreateTextureFromFile( pd3dDevice, strTexture
								, &m_pTextures[i] ) ) )
							{
								m_pTextures[i] = NULL;
