...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
    ...
    if( m_hWnd == NULL)
    {
		...
		WNDCLASS wndClass = { 0, WndProc, 0, 0, hInstance,
			LoadIcon( hInstance, MAKEINTRESOURCE(IDI_MAIN_ICON) ),
			LoadCursor(hInstance,MAKEINTRESOURCE(IDC_POINTER)), //09-08-07 이재준
			(HBRUSH)GetStockObject(WHITE_BRUSH),
			NULL, _T("D3D Window") };
...
EndWindowedDeviceComboSearch:
    ...
    if (m_d3dEnumeration.AppUsesDepthBuffer)
	{
		...
		m_d3dSettings.Windowed_DepthStencilBufferFormat = D3DFMT_D24S8;
	}
...
EndFullscreenDeviceComboSearch:
    ...
    if (m_d3dEnumeration.AppUsesDepthBuffer)
	{
		...
		m_d3dSettings.Fullscreen_DepthStencilBufferFormat = D3DFMT_D24S8;
	}
