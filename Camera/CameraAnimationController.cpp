...
CCameraAnimationController::CCameraAnimationController()
{
	m_pd3dDevice		= NULL;
	...
	m_eCameraPlayType			= ONETIME_PLAY;
	m_bAniFinished		= FALSE;
}
...
HRESULT		CCameraAnimationController::InitDeviceObjects( LPDIRECT3DDEVICE9 pDevice )
{
	HRESULT	hr	= S_OK;
...
HRESULT		CCameraAnimationController::InitDeviceObjects( LPDIRECT3DDEVICE9 pDevice, LPD3DXANIMATIONCONTROLLER pAniController )
{
	HRESULT	hr	= S_OK;
...
HRESULT		CCameraAnimationController::RestoreDeviceObjects()
{
	HRESULT	hr	= S_OK;
...
HRESULT		CCameraAnimationController::InvalidateDeviceObjects()
{
	HRESULT	hr	= S_OK;
...
HRESULT		CCameraAnimationController::DeleteDeviceObjects()
{
	HRESULT	hr	= S_OK;
...
LRESULT		CCameraAnimationController::HandleMessages( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	HRESULT	hr	= S_OK;
...
HRESULT		CCameraAnimationController::FrameMove( float	fElapsedTime )
{
	HRESULT	hr	= S_OK;
	...
	if( pos + 0.2f>= period  )
	{
		switch( m_eCameraPlayType )
		{
		case	ONETIME_PLAY:
			
			if(m_bAniFinished == FALSE)
			{
				m_pAniController->SetTrackPosition(0,0);
				m_pAniController->SetTrackPosition(1,0);

				m_pAniController->SetTrackPosition(m_currentTrack,period-0.2f);

				m_bAniFinished = TRUE;
			}
			break;
		case	LOOP_PLAY:
			m_pAniController->SetTrackPosition(m_currentTrack,0);
			break;
		}
...
HRESULT		CCameraAnimationController::TransitionAnimation( DWORD	dwNewSet )
{
	HRESULT	hr	= S_OK;
...
HRESULT		CCameraAnimationController::JumpAnimationSet( DWORD	dwNewSet )
{
	HRESULT		hr	= S_OK;

	LPD3DXANIMATIONSET	pAniSet	= NULL;

	hr	= m_pAniController->GetAnimationSet( dwNewSet, &pAniSet );
	CHECK_FAILED( hr );

	hr	= m_pAniController->SetTrackAnimationSet( m_currentTrack, pAniSet );
	CHECK_FAILED( hr );

	hr	= m_pAniController->SetTrackPosition( m_currentTrack, 0 );
	CHECK_FAILED( hr );

	hr	= m_pAniController->SetTrackEnable( m_currentTrack, TRUE );
	hr	= m_pAniController->SetTrackSpeed ( m_currentTrack, 1.f );
	hr	= m_pAniController->SetTrackWeight( m_currentTrack, 1.f );
